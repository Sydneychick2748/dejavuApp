...
# # Updated endpoint to save database as a ZIP file with proper cleanup
...
#     try:
#         # Create a temporary directory to build the folder structure
...
#             # Extract subfolder name and file name from the filename (e.g., "subfolder/file.txt")
...
#         # Create a ZIP file from the main folder
...
#         async def cleanup():
#             await asyncio.sleep(1)  # Brief delay to ensure file is sent
#             if temp_dir.exists():
...
async def extract_frames(
    file: UploadFile = File(...),
    timestamps: str = Form(None),
    save_frames: bool = Form(True)  # Default to saving frames
):
    try:
        video_id = str(uuid.uuid4())
        base_filename = os.path.basename(file.filename)
        ...
        if timestamps:
            ...
            # Extract all frames at the video's frame rate
            frame_pattern = str(frame_output_dir / "frame-%06d.jpg")
            stream = ffmpeg.input(str(transcoded_video))
            stream = ffmpeg.output(
                stream,
                frame_pattern,
            ...
            for i, frame_file in enumerate(sorted(frame_output_dir.glob("frame-*.jpg"))):
                ...
                frame_info = {
                    ...
                    "size_bytes": frame_stat.st_size,
                    "resolution": f"{metadata['width']}x{metadata['height']}",
                    "url": f"http://localhost:8000/frames/{video_id}/frame-{i:06d}.jpg"
                }
                frames.append(frame_info)
