...
async def reset_password(data: ResetPassword, db: Session = Depends(get_db)):
    logger.info(f"Database connection info: {db.bind.url}")
    logger.info(f"Reset password attempt for: {data.emailOrPhone}")
    ...
    if not user:
        logger.info(f"Reset password failed: User not found: {data.emailOrPhone}")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="User not found"
        )
    ...
    # Check if user is verified
    if not user.is_verified:
        logger.info(f"Reset password failed: User not verified: {data.emailOrPhone}")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="User is not verified. Please verify your account first."
        )
    ...
    logger.info(f"New password hashed for {user.email}")

    # Check if the new password matches the current password
    if pwd_context.verify(data.password, user.password):
        logger.info(f"Reset password failed: New password matches current password for {user.email}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="This password was previously used, please choose a different password"
        )
    ...
    try:
        db.execute(
            ...
            WHERE email = :email
            """),
            {"password": new_hashed_password, "email": user.email}
        )
        db.commit()
        logger.info(f"Password updated successfully for user: {user.email}")
    except Exception as e:
        logger.error(f"Database error during password update for {user.email}: {str(e)}")
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    logger.info(f"Post-reset user state: {user.email}, password updated: {updated_user.password != user.password}")
