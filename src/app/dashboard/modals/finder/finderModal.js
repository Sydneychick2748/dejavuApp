...
//   const locations = [
//     { name: "Photon Default Location",path: "/Photon", icon: <FaImage /> },
...
const FinderModal = ({ onClose, onSelectLocation, databaseName, savePath, items }) => {
  // Initialize selectedPath with savePath if available, otherwise default to /Photos
  const [selectedPath, setSelectedPath] = useState(savePath || "/Photon");
  const [showContents, setShowContents] = useState(false);
  ...
  // Update selectedPath when savePath changes (e.g., if parent component updates savePath)
  useEffect(() => {
    if (savePath) {
  ...
  // Construct the full path for display using selectedPath
  const fullPath = `${selectedPath}/${databaseName || "Unnamed Database"}`;
