...
//       if (!current || !current[indices[i]]) return null; // Return null if path is invalid
//       current = current[indices[i]];
//       // If we're not at the last index and there are subfolders, navigate into them
//       if (i < indices.length - 1) {
//         current = current.subFolders;
//       }
//     }
//     return current; // Return the folder object at the specified path
//   };
...
//     // Update child folder selections recursively
//     const updateChildSelections = (folders, parentPath) => {
//       folders.forEach((folder, index) => {
...
//         if (folder.files) {
//           const newFileSelections = { ...fileSelections };
//           newFileSelections[childPath] = folder.files.reduce((fileAcc, _, fileIndex) => {
//             fileAcc[fileIndex] = isChecked;
//             return fileAcc;
//           }, {});
//           setFileSelections(newFileSelections);
//         }
//         if (folder.subFolders && folder.subFolders.length > 0) {
//           updateChildSelections(folder.subFolders, childPath);
//         }
//       });
//     };
//     if (folder && folder.subFolders && folder.subFolders.length > 0) {
//       updateChildSelections(folder.subFolders, folderPath);
//     }
...
//     let count = 0;
//     const folder = getFolderByPath(folderPath);
//     if (folder && folder.files && folderSelections[folderPath]) {
...
//       const displayText = hasSubFolders
//         ? `${folder.subFolders.length} subfolder${folder.subFolders.length !== 1 ? "s" : ""}`
//         : `${getSelectedItemsCount(folderPath)} item${getSelectedItemsCount(folderPath) !== 1 ? "s" : ""} selected`;
...
//                   {displayText} {/* Display subfolders or items selected */}
//                 </div>
//               </div>
//             </div>
//             <div style={{ display: "flex", alignItems: "center" }}>
...
// DisplayDataBaseModal component to display and manage the database with a flattened list of files for viewing
const DisplayDataBaseModal = ({ onClose, onNext, selectedFolders }) => {
  // Access the ImageContext to set final selected images
  const { setFinalSelectedImages } = useContext(ImageContext);
  ...
  // Flatten all files from the folder structure into a single array
  const allFiles = [];
  const collectFilesRecursively = (folders) => {
    folders.forEach((folder) => {
      if (folder.files && folder.files.length > 0) {
        allFiles.push(...folder.files); // Add files from this folder
      }
      if (folder.subFolders && folder.subFolders.length > 0) {
        collectFilesRecursively(folder.subFolders); // Recursively collect files from subfolders
      }
    });
  };
  ...
  const [fileSelections, setFileSelections] = useState(() => {
    ...
    allFiles.forEach((file, index) => {
      initialFileSelections[index] = true; // Default to checked
    });
    return initialFileSelections;
  });
  ...
  const [isFilesExpanded, setIsFilesExpanded] = useState(false); // Control whether the file list is expanded

  // State for database settings
  const [resolutionLevel, setResolutionLevel] = useState(2); // Default resolution level
  const [colorSetting, setColorSetting] = useState("fullColor"); // Default color setting
  ...
  }, []); // Run once on mount, since allFiles is derived at initialization
  ...
  const toggleFileSelection = (fileIndex) => {
    ...
    // Update database selection
    const allFilesChecked = Object.values(fileSelections).every(Boolean);
  ...
  const handleFinish = () => {
    ...
    const databaseName = `database ${String.fromCharCode(65 + databaseCount)}`;
    setDatabaseCount((prev) => prev + 1); // Increment database count
    onNext({ files: selectedFiles, databaseName, resolutionLevel, colorSetting }); // Pass data to the next modal
  };
  ...
  return (
    <div className="modal" style={modalStyle}>
      ...
      <div style={{ display: "flex", width: "100%", height: "400px", flexDirection: "column" }}>
        {/* Database Header with Dropdown Arrow on the Right */}
        <div style={{ padding: "10px", borderBottom: "1px solid #ddd", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button
            ...
            style={toggleButtonStyle}
          >
            {isFilesExpanded ? <FaChevronUp /> : <FaChevronDown />} {/* Toggle arrow on the right */}
          </button>
        </div>

        {/* Files Section and Preview Panel */}
        <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
          {/* Left Side - Flattened List of Files (Hidden by Default, Expands on Dropdown) */}
          <div style={leftPanelStyle}>
            {isFilesExpanded && allFiles.length > 0 ? (
              allFiles.map((file, index) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      ...fileItemStyle,
                      opacity: fileSelections[index] ? 1 : 0.4,
                    }}
                  >
                    <input
                      type="checkbox"
              ...
              <p style={{ color: "#000000" }}>No files selected.</p> // Message if no files
            ) : null}
          </div>

          {/* Right Side - Image/Video Preview and Details (Always Visible) */}
          <div style={rightPanelStyle}>
            {selectedPreview ? (
              <>
                ...
                <div style={previewDetailsStyle}>
                  <p>
                    <strong>Name:</strong>{" "}
                    {selectedFile?.name || "Unknown"} {/* File name */}
                  </p>
                  <p>
                    <strong>Size:</strong>{" "}
                    {((selectedFile?.size || 0) / (1024 * 1024)).toFixed(2)} MB {/* File size */}
                  </p>
                  {selectedFile?.type.startsWith("video/") ? (
                    <>
                      <p>
                        <strong>Resolution:</strong>{" "}
                        {videoRef.current ? `${videoRef.current.videoWidth}x${videoRef.current.videoHeight}` : "N/A"} {/* Video resolution */}
                      </p>
                      <p>
                        <strong>Duration:</strong>{" "}
                        {formatTime(videoDuration)} {/* Video duration */}
                      </p>
                    </>
                  ) : (
                    <p>
                      <strong>Resolution:</strong> N/A {/* Resolution not applicable for images */}
                    </p>
                  )}
                  <p style={{ color: "#007BFF", cursor: "default" }}>
                    Open File {/* Label for file details */}
                  </p>
                </div>
              </>
            ) : (
        ...
        {/* Database Settings Bar at the Bottom */}
