...
//   const [selectedFileIndex, setSelectedFileIndex] = useState(null); // Track selected file index for frame info

//   // State to track database name sequence
//   const [databaseCount, setDatabaseCount] = useState(() => {
//     // Load from localStorage or start at 1 (for 'B' as the first database)
...
//   // Calculate total frame count for videos (simplified, assuming frame count is in file name or metadata)
//   const getFrameInfo = (file) => {
//     // This is a placeholder; adjust based on actual frame count data
//     const frameCountMatch = file.name.match(/\| (\d+) frames/);
//     const frameCount = frameCountMatch ? parseInt(frameCountMatch[1], 10) : 0;
//     return frameCount > 0 ? `Frame 001 / ${frameCount}` : "";
//   };
...
//                                 setSelectedFileIndex(fileIndex); // Track selected file index
//                               }}
//                             />
//                           ) : (
...
//                 <p><strong>Name:</strong> {selectedFolders.flatMap(folder => folder.files)[selectedFileIndex]?.name || "Unknown"}</p>
//                 <p><strong>Size:</strong> {(selectedFolders.flatMap(folder => folder.files)[selectedFileIndex]?.size / (1024 * 1024)).toFixed(2)} MB</p>
//                 <p><strong>Frame:</strong> {getFrameInfo(selectedFolders.flatMap(folder => folder.files)[selectedFileIndex] || {})}</p>
//                 <p
...
//   borderRadius: "8px",
//   zIndex: 1000,
//   width: "800px", // Adjusted for better layout
//   height: "500px",
// };
