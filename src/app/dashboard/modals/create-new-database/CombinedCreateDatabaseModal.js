...
const CombinedCreateDatabaseModal = ({ onClose, onFinish, existingDatabaseNames }) => {
  ...
  const [saveLocationError, setSaveLocationError] = useState(""); // State for save location error
  const [duplicateFolderError, setDuplicateFolderError] = useState(""); // State for duplicate folder error
  ...
  const handleFolderChange = (event) => {
    ...
    const existingNames = new Set(selectedFolders.map((f) => f.name));
    const uniqueNewFolders = newFolders.filter((f) => !existingNames.has(f.name));
    const duplicateFolders = newFolders.filter((f) => existingNames.has(f.name));

    if (duplicateFolders.length > 0) {
      setDuplicateFolderError("You have already added this folder.");
      return;
    }

    setDuplicateFolderError(""); // Clear any previous duplicate folder error
    setSelectedFolders((prev) => [...prev, ...uniqueNewFolders]);
  ...
  const handleUploadClick = () => {
    ...
    setDuplicateFolderError("");

    // Collect validation errors
    let hasErrors = false;

    // Check database name (required in both modes)
    if (!databaseName.trim()) {
      setImportMediaError("Please enter your database name first.");
      hasErrors = true;
    }

    // Check save location (required in manual mode)
    if (databaseLocation === "manual" && !saveLocation.trim()) {
      setSaveLocationError("Please select a save location before importing media.");
      hasErrors = true;
    }
  ...
  const handleMoreMediaUploadClick = () => {
    // Since "Import more media" only appears after media is uploaded,
    // and "Import media" validation ensures both fields are filled,
    // we don't need validation here for database name and save location.
    setImportMediaError("");
    setSaveLocationError("");
    setDuplicateFolderError("");
    setIsDatabaseNameLocked(true);
  ...
  const handleSelectLocation = (path) => {
    ...
    if (saveLocationError && path.trim()) {
      setSaveLocationError("");
    }
  };
    ...
    <div className="modal" style={styles.modalStyle}>
      ...
      <div style={styles.contentStyle}>
        <div className="input-container">
            ...
            <input
              ...
              onChange={(e) => {
                if (!isDatabaseNameLocked) {
                  ...
                  if (importMediaError && e.target.value.trim()) {
                    setImportMediaError("");
                  }
                }
              }}
          ...
          {/* Explanatory text below Database Name, hidden in manual mode */}
          {databaseLocation === "default" && (
            <p style={{ fontSize: "0.9rem", color: "#555", marginTop: "5px" }}>
              By default, databases are saved to the default Photon directory on your local system. This can be changed in Preferences.
            </p>
          )}
        </div>
        ...
        {databaseLocation === "manual" && (
          <div className="input-container">
            ...
            <div className="input-group">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (saveLocationError && e.target.value.trim()) {
                    setSaveLocationError("");
                  }
                }}
            ...
            {saveLocationError && <div className="input-error">{saveLocationError}</div>}
          ...
          <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
            <div className="media-import-left left-panel">
              {selectedFolders.length > 0 ? (
                ...
                <div className="import-media-button-container">
                  ...
                  {duplicateFolderError && <div className="input-error">{duplicateFolderError}</div>}
                  <input
                    type="file"
                    ref={folderInputRef}
            ...
            {selectedFolders.length > 0 && (
              <div className="import-more-media-container">
                <button
                  onClick={handleMoreMediaUploadClick}
                  className="import-media-button"
                >
                  Import more media
                </button>
        ...
        <div className="button-container">
          ...
          <button
            ...
            className={`create-button ${selectedFolders.length > 0 ? "enabled" : "disabled"}`}
