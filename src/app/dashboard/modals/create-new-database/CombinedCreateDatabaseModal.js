...
//   const [isDatabaseNameLocked, setIsDatabaseNameLocked] = useState(false); // New state to lock the database name input
...
//     // Lock the database name input if a name is provided
...
//     if (videoRef.current) {
//       videoRef.current.pause(); // Pause the video while seeking
//     }
//   };
...
//         console.error("Error playing video after seeking:", error);
//       });
//     }
//   };
...
// src/app/dashboard/modals/create-new-database/CombinedCreateDatabaseModal.js
...
const CombinedCreateDatabaseModal = ({ onClose, onFinish, existingDatabaseNames }) => {
  ...
  const handleFolderChange = (event) => {
    ...
    files.forEach((file) => {
      // Debug file types to identify why PNG and JPG aren't showing
      console.log(`File: ${file.name}, Type: ${file.type}`);

      // Relaxed file type check to include common image and video types
                        ...
                        >
                          ...
                          {fileUrl && file.type && file.type.toLowerCase().startsWith("image/") ? (
                            ...
                            <video
                              ...
                              muted
                              onLoadedMetadata={handleVideoLoadedMetadata}
                              onClick={handlePreviewClick} // Added onClick to make video thumbnail clickable
                            >
                              Your browser does not support the video tag.
                            </video>
                          ) : (
