        ...
        <div className="image-content">
          ...
          <div className="image-details">
            <div className="file-info-wrapper">
              ...
              <p
                className="file-info-link"
                onClick={() => setShowFileInfoModal(box)} // Pass the box data to the modal
              >
                File Info
              </p>
              <p
            ...
            <div className="buttons-wrapper">
              <button
                onClick={() => handleIsolateSubject(box.id)}
                className="action-button"
                disabled={box.isSubjectIsolated} // Disable if subject is isolated
                style={box.isSubjectIsolated ? { backgroundColor: "#ccc", cursor: "not-allowed" } : {}}
              >
                Isolate Subject
              </button>
              <button
                onClick={() => openMaskEditor(box.id)}
                className="action-button"
                disabled={box.isManuallyMasked} // Disable if manually masked
                style={box.isManuallyMasked ? { backgroundColor: "#ccc", cursor: "not-allowed" } : {}}
              >
                Manual Mask
              </button>
              <button className="action-button">Create New Object</button>
...
export default function SearchFor() {
  ...
  const [response, setResponse] = useState("");
  const [isMaskEditorOpen, setIsMaskEditorOpen] = useState(false);
  const [showFileInfoModal, setShowFileInfoModal] = useState(null); // Store box data instead of boolean
  const [showOpenFileModal, setShowOpenFileModal] = useState(false);
  const [boxes, setBoxes] = useState([
    {
      id: Date.now(),
      ...
      fileSize: 0,
      fileType: "",
      isSubjectIsolated: false, // Track if subject has been isolated
      isManuallyMasked: false,  // Track if manual mask has been applied
    },
  ]);
  const [lastProcessedFile, setLastProcessedFile] = useState(null);
  const canvasRef = useRef(null);
  ...
  const handleCloseModal = (boxId, imageInfo) => {
    if (imageInfo) {
      // Optional: Add warning for duplicates (uncomment to enable)
      /*
      const isDuplicate = boxes.some(
        (box) => box.selectedImage && box.fileName === imageInfo.fileName
      );
      if (isDuplicate) {
        const proceed = window.confirm(
          `The image "${imageInfo.fileName}" is already added to another box. Do you want to add it again?`
        );
        if (!proceed) {
          setIsModalOpen(false);
          return;
  ...
  const handleIsolateSubject = async (boxId) => {
    ...
    try {
      ...
      const res = await fetch("http://127.0.0.1:8000/dashboard/process-image", {
        method: "POST",
        body: formData,
      });
      if (!res.ok) throw new Error(`Upload failed with status: ${res.status}`);
      ...
      setResponse(data.message);
      if (data.filename) {
        const processedUrl = `http://127.0.0.1:8000/dashboard/get-image/${encodeURIComponent(
          data.filename
        )}`;
        setBoxes((prevBoxes) => {
          const newBoxes = JSON.parse(JSON.stringify(prevBoxes));
          ...
          if (boxIndex !== -1) {
            newBoxes[boxIndex] = {
              ...
              imageUrl: processedUrl,
              baseImageUrl: processedUrl,
              isSubjectIsolated: true, // Mark as isolated
            };
          }
          console.log(
            `Updated box ${boxId} with processed image: ${processedUrl}`
  ...
  const handleManualMask = async () => {
    ...
    try {
      ...
      const res = await fetch("http://127.0.0.1:8000/dashboard/remove-area", {
        method: "POST",
        body: formData,
      });
      if (!res.ok) throw new Error(`Upload failed with status: ${res.status}`);
        ...
        setBoxes((prevBoxes) => {
          ...
          if (boxIndex !== -1) {
            newBoxes[boxIndex] = {
              ...
              imageUrl: processedUrl,
              baseImageUrl: processedUrl,
              isManuallyMasked: true, // Mark as manually masked
            };
          }
          console.log(
            `Updated box ${currentBoxId} with processed image: ${processedUrl}`
        ...
        <div style={nestedModalOverlayStyle}>
          <div style={nestedModalContentStyle}>
            <div style={blueBarStyle}>
              <h3 style={{ color: "black", margin: 0, fontSize: 15 }}>File Info</h3>
              <button
                onClick={() => setShowFileInfoModal(null)}
                style={nestedModalCloseButtonStyle}
              >
                ×
              </button>
            ...
            <div style={nestedModalBodyStyle}>
              <p><strong>File Name:</strong> {showFileInfoModal.fileName}</p>
              <p><strong>File Type:</strong> {showFileInfoModal.fileType}</p>
              <p><strong>File Size:</strong> {(showFileInfoModal.fileSize / (1024 * 1024)).toFixed(2)} MB</p>
              <p><strong>Resolution:</strong> {showFileInfoModal.imgWidth}x{showFileInfoModal.imgHeight}</p>
