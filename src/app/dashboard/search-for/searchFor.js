...
//                 cursor:
//                   finalSelectedImages.length > 0 ? "pointer" : "not-allowed",
...
export default function SearchFor() {
  const [imageUrl, setImageUrl] = useState("");
  const [baseImageUrl, setBaseImageUrl] = useState("");
  const { selectedImage, setSelectedImage, finalSelectedImages } = useContext(ImageContext);
  const [isModalOpen, setIsModalOpen] = useState(false);
  ...
  return (
    <div style={{ padding: "20px", color: "black" }}>
      {!selectedImage ? (
        <div style={{  padding: "20px", color: "black", display: "flex", alignItems: "center", justifyContent: "space-between", background: "#EDEDED", padding: "15px", borderRadius: "10px" }}>
          <div
            style={{
              width: "250px",
              height: "250px",
